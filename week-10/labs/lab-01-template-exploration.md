...
## Part B — Duplicate the User Template and Configure for Web Server Use

> **Why User, not Web Server?** The Web Server template is a machine-type template at the Windows COM layer — even after attribute changes, the certificate enrollment wizard will refuse to issue it to a user account. Duplicating from the **User** template produces a user-type template that the enrollment wizard accepts. We then add Server Authentication EKU manually to give it the web server purpose.

### Step 1 — Open the Duplicate Dialog

1. In `certtmpl.msc`, locate the **User** template
2. Right-click **User** → **Duplicate Template**
3. The Duplicate Template dialog appears

**Source template used for duplication:** ________________ (document: User)
...
|---------|---------------|-----------|
| Template display name | User | CVI-WebServer |
| Template name (internal) | User | CVI-WebServer |
...
### Step 4 — Configure Extended Key Usage (Extensions Tab)

The User template includes EKUs for Client Authentication, Encrypting File System, and Secure Email. For CVI-WebServer, we need Server Authentication and Client Authentication only.

1. Click the **Extensions** tab
2. Select **Application Policies** → click **Edit**
3. Remove **Encrypting File System** — select it → click **Remove**
4. Remove **Secure Email** — select it → click **Remove**
5. Add **Server Authentication** — click **Add** → scroll to find **Server Authentication** (OID `1.3.6.1.5.5.7.3.1`) → select it → click **OK**
6. Confirm the list now shows exactly:
...
| Server Authentication | | 1.3.6.1.5.5.7.3.1 |
| Client Authentication | | 1.3.6.1.5.5.7.3.2 |
| Encrypting File System | | 1.3.6.1.5.5.7.3.4 |
| Secure Email | | 1.3.6.1.5.5.7.3.4 |

**Why keep Client Authentication alongside Server Authentication?**

```
(your explanation here — why does a web server certificate benefit from including Client Authentication?)
```

### Step 5 — Configure the Subject Name Tab

The User template defaults to **Build from this Active Directory information**. CVI-WebServer needs the subject name supplied at request time because the certificate CN (e.g., `webserver.corp.cvilab.local`) is not an AD attribute.

1. Click the **Subject Name** tab
2. Select **Supply in the request** (change from the default "Build from this Active Directory information")
3. Click **OK** on any warning dialog that appears

**Supply in the request is selected:**
- [ ] Yes

**Why does CVI-WebServer use "Supply in the request" rather than building from AD?**

```
(your explanation here — what does AD know about this server, and what does it not know?)
```

### Step 6 — Configure Security Tab Permissions

The User template gives Authenticated Users Read access only. pki.admin needs Enroll permission to request this certificate.

1. Click the **Security** tab
2. Confirm **Authenticated Users** have **Read** (not Enroll) — leave as-is
3. Add **pki.admin**:
   - Click **Add**
   - Type `pki.admin` → click **Check Names** → confirm it resolves to `CORP\pki.admin` → click **OK**
   - With pki.admin selected, check **Read** and **Enroll**
4. Click **Apply**
...
**Why is Enroll restricted to pki.admin rather than all Authenticated Users?**

```
(your explanation here — what is the least-privilege principle and how does it apply here?)
```
...
**Why did we duplicate from the User template rather than the Web Server template, even though CVI-WebServer is for web server certificates?**

```
(your answer here — consider the difference between template type at the Windows enrollment layer vs. the EKU purpose of the certificate)
```
...
- [ ] Part B: Duplicate dialog opened from **User** template
...
- [ ] Part B: EKU configured — Server Auth added, EFS and Secure Email removed, Client Auth retained
- [ ] Part B: Subject Name changed to "Supply in the request" — rationale provided
- [ ] Part B: pki.admin added with Read + Enroll — Security tab documented
